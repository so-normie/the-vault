...
export default class ObsidianFlashcard extends Plugin {
	...
	async onload() {
		...
		this.addCommand({
			id: 'remove-flashcard-ids-from-the-current-file',
			name: 'Remove generated flashcard IDs from the current file',
			checkCallback: (checking: boolean) => {
				const activeFile = this.app.workspace.getActiveFile()
				if (activeFile) {
					if (!checking) {
						console.log("remove flashcard ids from current file")
					}
					return true;
				}
				return false;
